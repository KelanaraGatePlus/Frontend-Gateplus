...
import logoEducation from "@@/logo/logo-gateplus-education.png";
...
export default function NavbarEduacation({ openCreateContentModal, openRedeemVoucherModal }) {
    ...
    const handleSearchSubmit = () => {
        if (searchQuery.trim()) {
            router.push(`/education?search=${encodeURIComponent(searchQuery)}`);
            ...
            >
                <section className={`${isSearchActive ? "hidden md:flex" : "flex"} items-center justify-between p-1 2xl:p-4 md:flex md:justify-between md:bg-fixed`}>
                    <div className="-mt-3 w-fit -translate-x-8 ps-0 md:mt-0 md:-translate-x-0 md:place-items-start lg:px-6">
                        <Link href="/education"><div className="flex aspect-auto justify-center"><Image className="ml-6 h-auto w-auto" src={logoEducation} alt="logo-gate+" priority /></div></Link>
                    </div>

                    {!isSearchActive ? (
                        <div className="flex flex-row gap-12">
                            <Link href={'/education'} className="zeinFont mt-0.5 flex items-center w-fit justify-between gap-1 2xl:gap-3 rounded-full px-4 py-1 2xl:py-2 text-sm 2xl:text-xl leading-tight transition-all duration-300 ease-in-out md:flex md:bg-[#0881AB] lg:-mr-10">
                                <Icon
                            ...
                            {/* <div className="zeinFont mt-0.5 hidden w-fit justify-between gap-1 2xl:gap-3 rounded-full px-2 py-1 2xl:py-2 text-sm 2xl:text-xl leading-tight transition-all duration-300 ease-in-out md:flex md:bg-[#0881AB] lg:-mr-10">
                                <Link href={'/education/saved-courses'}>
                                    <div className={`flex cursor-pointer items-center justify-center rounded-full border-2 border-[#0881AB] from-[#0E5BA8] to-[#0395BC] p-2 text-center font-semibold text-white lg:px-3 xl:px-4 hover:bg-linear-to-t active:bg-linear-to-t active:border-white/40`}>
                                        <span className="drop-shadow-[0_0_2px_rgba(255,255,255,0.3)]">Course Tersimpan</span>
                                    </div>
                                </Link>
                                <Link href={'/education/my-courses'}>
                                    <div className={`flex cursor-pointer items-center justify-center rounded-full border-2 border-[#0881AB] from-[#0E5BA8] to-[#0395BC] p-2 text-center font-semibold text-white lg:px-3 xl:px-4 hover:bg-linear-to-t active:bg-linear-to-t active:border-white/40`}>
                                        <span className="drop-shadow-[0_0_2px_rgba(255,255,255,0.3)]">Course Berjalan</span>
                                    </div>
                                </Link>
                            </div> */}
                            <button onClick={() => setIsSearchActive(true)} className="zeinFont mt-0.5 flex items-center w-fit justify-between gap-1 2xl:gap-3 rounded-full px-6 border border-[#F5F5F526] py-1 2xl:py-2 text-sm 2xl:text-xl leading-tight transition-all duration-300 ease-in-out md:flex md:bg-[#F5F5F526] lg:-mr-10">
                                <Icon
                                    icon={'lucide:search'}
                                    className="text-white"
                                    width={28}
                    ...
                    <div className="flex w-fit items-center justify-end gap-4 text-white md:flex md:w-auto md:gap-2 lg:gap-3">
                        ...
                        {isAuthenticated ? (
                            <div className="flex flex-row gap-2 items-center">
                                <Link href={'/'} className="zeinFont flex h-max items-center w-fit justify-between gap-1 2xl:gap-3 rounded-full px-6 py-1 2xl:py-2 text-sm 2xl:text-xl leading-tight transition-all duration-300 ease-in-out md:flex bg-[#0881AB] hover:bg-[#066d8f]">
                                    <Icon
                                        icon={'solar:devices-outline'}
                                        className="text-white"
                                        width={28}
                                        height={28}
                                    />
                ...
                >
                    ...
                    <div className="grid grid-cols-1 gap-2 p-4 text-xs">
                        <div>
                            ...
                            <div className="p-1">
                                <ul className="space-y-1">{trendingData?.popularSearches?.map((item, index) => (<li key={index}><Link href={`/education?search=${item.searchQuery}`} className="text-white hover:text-white flex flex-row items-center gap-1">
                        ...
                        <div>
                            ...
                            <div>
                                <div className="px-2 py-1">
                                    <ul className="space-y-1">{data?.searchHistories?.map((item, index) => (<li key={index}><Link href={`/education?search=${item.searchQuery}`} className="text-white hover:text-white">{item.searchQuery}</Link></li>))}</ul>
                                </div>
                            </div>
                        </div>
                    </div>
