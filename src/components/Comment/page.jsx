...
export default function CommentComponent({
  ...
  return (
    <section
      className={`flex flex-col text-white transition-all duration-200 ease-in-out ${isCommentVisible ? "my-4 w-1/2 pr-5" : "w-full px-5 py-6"} ${isExpand ? "h-screen" : "h-full"}`}
    >
      <div
        className={`sticky top-0 z-10 flex w-full flex-col bg-[#171717] px-4`}
      >
        <div className="relative flex w-full justify-start px-4 py-2">
          <h3 className={`zeinFont ml-4 text-3xl font-extrabold`}>Komentar</h3>
          <button
            className="absolute top-1/2 left-0 -mt-2 flex -translate-y-1/2 cursor-pointer text-4xl"
            onClick={handleViewComments}
          >
            &times;
          </button>
          <button
            className="montserratFont absolute top-1/2 right-0 -mt-2 flex cursor-pointer text-xs text-white/50"
      ...
      <div className="custom-scrollbar mb-10 flex flex-col gap-4 overflow-x-hidden">
