...
export default function ProductDetailSection({
  ...
  const handleToggleLike = () => {
    likeProduct(isLiked, productTitle, productID, "ebookId", totalLike, {
      setShowToast,
      setToastMessage,
      setToastType,
      setIsLiked,
  ...
  const handleToggleSave = () => {
    saveProduct(isSaved, productTitle, productID, "ebookId", {
      setShowToast,
      setToastMessage,
      setToastType,
      setIsSaved,
                  ...
                  <div className="group flex max-w-36 cursor-pointer flex-col rounded-lg text-ellipsis whitespace-nowrap md:max-w-72">
                    ...
                    <p className="-mt-1 text-[10px] font-light md:text-sm">
                      {formatFollowersCount(
                        creatorTotalSubscriber ? creatorTotalSubscriber : 0,
                      )}{" "}
                      {creatorTotalSubscriber > 1 ? " Followers" : " Follower"}
                    </p>
                  </div>
                </Link>
                {!isOwnChannel && (
                  <button
                    className={`zeinFont mt-1 flex cursor-pointer items-center justify-center rounded-full ${!creatorIsSubscribed ? "bg-blue-800 hover:bg-blue-900" : "bg-gray-600 hover:bg-gray-700"} px-5 pt-1.5 pb-1 text-xl`}
