...
export default function EbookCard({ title, id, coverUrl, rank = null, isOriginal = false, hasNewEpisode = false, withTopTag = true, withNewestTag = false }) {
    return (
        <Link href={`/${contentType.ebooks.pluralName}/detail/${id}`} className="h-full w-full">
            <div className="relative h-full w-full rounded-[6px] overflow-hidden group">
                ...
                <div className="absolute top-0 left-0 w-full flex justify-between items-center z-20 px-2 py-1">
                    ...
                    >
                        {rank && withTopTag && <div className="flex flex-col rounded-sm bg-[#22222233] backdrop-blur-xs py-1 px-4 font-black items-center text-cyan-400 zeinFont">
