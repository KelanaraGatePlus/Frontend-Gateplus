...
export default function PaymentCheckoutPage({ params }) {
    ...
    return (
        <div className="w-full h-max flex justify-center items-center mt-10">
            {isShowInput && <div className="bg-[#515151] drop-shadow-md drop-shadow-[#0000004D] sm:w-min-w-2xl md:min-w-3xl xl:min-w-5xl 2xl:min-w-6xl text-xs md:text-[16px] max-w-max rounded-md montserratFont text-white">
                <div className="px-8 pt-4 pb-10 flex flex-col gap-8">
                    ...
                    <div className="flex flex-col gap-2 md:px-8">
                        ...
                        {!selectedPaymentMethod && <p className="text-red-600 text-sm font-semibold">Pilih metode pembayaran terlebih dahulu</p>}
                    </div>

                    {/* Voucher & Tip Section */}
                    <div className="flex flex-col px-0 md:px-8 gap-8">
                        <div className="flex flex-col gap-2 overflow-hidden">
                            {price && <div className="flex md:flex-row flex-col bg-[#DEDEDE4D] rounded-l-full overflow-hidden">
                                <button disabled={!voucherCode} onClick={() => handleApplyVoucher(voucherCode, Number(price))} className="px-6 md:px-12 py-3 bg-[#0075e9c4] rounded-full font-semibold whitespace-nowrap hover:cursor-pointer">
                                    Gunakan Voucher
                                </button>
                                <input
                                    type="text"
