...
export const discountVoucherAPI = createApi({
    ...
    endpoints: (builder) => ({
        getDiscountByVoucherDiscountCode: builder.mutation({
            query: ({ code, amount, contentType = null, contentId = null, paymentType }) => ({
                url: `/discountVoucher/count-discount/${code}/${amount}${contentType ? `?contentType=${contentType}` : ''}${contentId ? `&contentId=${contentId}` : ''}${paymentType ? `&paymentType=${paymentType}` : ''}`,
                method: "GET",
            }),
